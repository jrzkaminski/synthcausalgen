...
class SyntheticDataGenerator:
    def __init__(self, layer_structure, root_model_pool=None, leaf_model_pool=None, noise_model_pool=None, max_parents=None, root_params=None):
        self.layer_structure = layer_structure
        self.root_model_pool = root_model_pool if root_model_pool is not None else _CONTINUOUS_DISTRIBUTIONS
        self.leaf_model_pool = leaf_model_pool if leaf_model_pool is not None else [
            LinearLeafModel,
            PolynomialLeafModel,
            ExponentialLeafModel,
            LogarithmicLeafModel,
        ...
        self.noise_model_pool = noise_model_pool if noise_model_pool is not None else _CONTINUOUS_DISTRIBUTIONS
    ...
    def _initialize_root_features(self, n_root_features):
        for i in range(n_root_features):
            node_name = f"root_{i}"
            model = RootDistributionModel(model_pool=self.root_model_pool, params=self.root_params.get(node_name))
    ...
    def _generate_leaf_nodes(self, layer, n_leaf_nodes):
        previous_layers_nodes = [node for node, attr in self.graph.nodes(data=True) if attr['layer'] < layer]
        for i in range(n_leaf_nodes):
            node_name = f"leaf_{layer}_{i}"
            num_parents = random.randint(1, min(len(previous_layers_nodes), self.max_parents) if self.max_parents else len(previous_layers_nodes))
    ...
    def get_dataframe(self, size):
        ...
        for layer in sorted(self.layer_structure.keys()):
            nodes_in_layer = [node for node, attr in self.graph.nodes(data=True) if attr['layer'] == layer]
            for node in nodes_in_layer:
                data[node] = self._generate_data_for_node(node, data, size)
        columns = {node: f"node_l_{self.graph.nodes[node]['layer']}_p_{'_'.join(self.node_descriptions[node]['parents'])}" if self.node_descriptions[node]['type'] == 'leaf' else f"node_l_{self.graph.nodes[node]['layer']}" for node in data}
